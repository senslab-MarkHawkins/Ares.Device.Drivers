...
public class SimTicConnection(string portName) : AresSerialSimConnection(new SerialPortConnectionInfo(9600, Parity.None, 8, StopBits.One), portName)
{
  ...
  public override void SendInternally(byte[] bytes)
  {
    ...
    else if(bytes[0] == 0xE0 && bytes.Length >= 6)
      _targetPosition = bytes[1..6].FromPololuByteArray();

    // Halt and Set Position
    else if(bytes[0] == 0xEC && bytes.Length >= 6) 
    {
      _targetPosition = bytes[1..6].FromPololuByteArray();
      _currentPosition = _targetPosition;
    }

    else if(bytes[0] == 0xEA && bytes.Length >= 6) 
      _maxAcceleration = (uint)bytes[1..6].FromPololuByteArray(); 
    
    else if(bytes[0] == 0xE9 && bytes.Length >= 6) 
      _maxDeceleration = (uint)bytes[1..6].FromPololuByteArray();
    
    else if(bytes[0] == 0xE6 && bytes.Length >= 6)
      _maxSpeed = (uint)bytes[1..6].FromPololuByteArray();

    else if(bytes[0] == 0xE5 && bytes.Length >= 6) 
      _startingSpeed = (uint)bytes[1..6].FromPololuByteArray();

    else if(bytes[0] == 0x91 && bytes.Length >= 2)
      _currentLimit = bytes[1];
    ...
    if(bytes[0] == 0xA1) // Variable Request
    {
      ...
      switch(offset)
      {
        case 0x00: 
          response[0] = 10; 
          break; // OperationState.Normal

        case 0x01: 
          response[0] = (byte)(_energized ? 1 : 0); 
          break; // Misc Flags (Energized is bit 0)
        ...
        case 0x1E: 
          BitConverter.GetBytes(_maxAcceleration).CopyTo(response, 0); 
          break;

        case 0x1A:
          BitConverter.GetBytes(_maxDeceleration).CopyTo(response, 0); 
          break;

        case 0x16: 
          BitConverter.GetBytes(_maxSpeed).CopyTo(response, 0); 
          break;

        case 0x12: 
          BitConverter.GetBytes(_startingSpeed).CopyTo(response, 0); 
          break;

        case 0x4A: 
          BitConverter.GetBytes(_currentLimit).CopyTo(response, 0); 
          break;
      }
