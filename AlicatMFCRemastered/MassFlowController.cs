...
public class MassFlowController : AresDevice, IMassFlowController
{
        ...
        _stateWatchers = new CompositeDisposable
    {
      _serialConnection.GetTransactionStream<LiveDataResponse>().Select(transaction => transaction.Response).Subscribe(UpdateLiveData)
        ...
        MassFlowControllerRegistry.Register(this);
    }
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        MassFlowControllerRegistry.Unregister(this);
        ...
        try
        {
            await _stateUpdater.ConfigureAwait(false);
        }
        catch (OperationCanceledException)
        {
        ...
        if (_serialConnection is IAsyncDisposable asyncDisposable)
        {
            await asyncDisposable.DisposeAsync()
                .ConfigureAwait(false);
        }
        else if (_serialConnection is IDisposable disposable)
        {
            disposable.Dispose();
        }
    }
